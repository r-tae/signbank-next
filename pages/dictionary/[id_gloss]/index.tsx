  ...
  return (
    <main className="max-w-6xl overflow-visible px-16 xl:px-0">
      <Head>
        ...
        <title>
          {data.data.keywords[0].text.charAt(0).toUpperCase() +
            data.data.keywords[0].text.slice(1)}{' '}
          | Auslan Signbank
          {/* TODO: use website name from config/envvars */}
        </title>
      </Head>
      ...
      <div className="flex flex-col gap-4 md:flex-row">
        <div className="min-w-[36ch]">
          ...
          <div className="mb-4">
            ...
            {data.data.keywords.map((x, index) => (
              <span key={x.text}>
                {index ? ', ' : ''}
              ...
              {data.data?.relations
                ...
                .map(({ entry }) => (
                  <video
                    ...
                    src={`${staticFilesHost}${entry?.videoUrl}`}
                  />
                ))}
              {/* {data.data?.relations?.filter(relation => relation.role === "variant").map((variant) => (
                <video
                  key={variant.entry.idGloss}
                  className="mb-1 mt-10 w-44 rounded bg-gray-300 md:mt-0"
                  src={`${staticFilesHost}${
                    variant?.entry.videoUrl
                  }`}
                />
              ))} */}
            </div>
          ...
          <RegionInfoBox
            region={data.data.language.region}
            traditional={data.data.language.traditional}
            languageCode={data.data.language.code}
          />
        </div>
        <div className="flex flex-col gap-2">
          <div className="flex h-10 flex-row justify-end gap-x-4 py-1">
            <Button className="mr-auto" onClick={handleSwitchToDetailView}>
